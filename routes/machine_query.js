    ...
    app.post('/api/query/machine/', function(req, res){
        ...
        if(req.body.token && rsp){
            ...
            if(!req.body.myip || !req.body.atackip){
                return res.send({"status": "error", "code":21 ,"message": rlang(lang, 21)});
            }
            my.query('SELECT * FROM 4h_machines WHERE ip=?', req.body.myip, function (error, own_machine, fields) {
                if (error) {
                    console.log(error);
                    return res.send({"status": "error", "code":15 ,"message": rlang(lang, 15)});
                }else if(own_machine.lenght <= 0){
                    return res.send({"status": "error", "code":19 ,"message": rlang(lang, 19)});
                }else{
                    ...
                    if(own_machine.id_user == global.loggedUsers.get(req.body.token).id_user){
                        //Fazer join para pegar o balance
                        //Checa os bots
                        ...
                        if(atacked){
                            if(own_machine.scan > atacked.firewall){
                                ...
                                return res.send({"status": "error", "code":20 ,"message": rlang(lang, 20)});
                            }
                        }else{
                            my.query('SELECT * FROM 4h_machines WHERE ip=?', req.body.atackip, function (error, atacked, fields) {
                                //Checa se os status da sua maquina vs atacado
                                if(error){//Avisa se a machine com este ip não existe
                                    return res.send({"status": "error", "code":15 ,"message": rlang(lang, 15)});
                                }else if(atacked.lenght == undefined || atacked.lenght <= 0){
                                    return res.send({"status": "error", "code":19 ,"message": rlang(lang, 19)});
                        ...
                        return res.send({"status": "error", "code":18 ,"message": rlang(lang, 18)});
                    }
                }
            });
        }else{
